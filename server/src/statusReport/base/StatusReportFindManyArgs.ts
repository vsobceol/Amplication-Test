/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { StatusReportWhereInput } from "./StatusReportWhereInput";
import { Type } from "class-transformer";
import { StatusReportOrderByInput } from "./StatusReportOrderByInput";

@ArgsType()
class StatusReportFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => StatusReportWhereInput,
  })
  @Field(() => StatusReportWhereInput, { nullable: true })
  @Type(() => StatusReportWhereInput)
  where?: StatusReportWhereInput;

  @ApiProperty({
    required: false,
    type: [StatusReportOrderByInput],
  })
  @Field(() => [StatusReportOrderByInput], { nullable: true })
  @Type(() => StatusReportOrderByInput)
  orderBy?: Array<StatusReportOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { StatusReportFindManyArgs as StatusReportFindManyArgs };
